...
- Closeout for the active pack is forbidden unless this README and `docs/plan/data-dyna-vibecoding-guardrails_WORKSET.md` parse as active slice `VIBE_PACK_COMPLETE`, owner `closeout`, state `DONE`, and no non-deferred stages remain.
- If `closeout` is dispatched while `Current Active Slice` is anything other than `VIBE_PACK_COMPLETE`, treat it as a premature scheduler route and hand back to the active slice owner.
...
- Active-slice phase reports should use `stepId = DD-VIBE-S1` while this active slice remains current.
